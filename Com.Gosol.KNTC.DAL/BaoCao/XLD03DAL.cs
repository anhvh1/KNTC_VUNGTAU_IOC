    ...
    public class XLD03DAL
    {
        public List<TableData> XLD03(List<int> listCapChonBaoCao, string ContentRootPath, int RoleID, int CapID, int CoQuanDangNhapID, int CanBoDangNhapID, int TinhDangNhapID, int HuyenDangNhapID, DateTime startDate, DateTime endDate)
        {
            ...
            foreach (DataRow dro in dt.Rows)
            {
                ...
                double col13 = Utils.ConvertToDouble(dro["Col11Data"], 0);
                double col18 = Utils.ConvertToDouble(dro["Col15Data"], 0);
                double col19 = Utils.ConvertToDouble(dro["Col16Data"], 0);
                double col20 = Utils.ConvertToDouble(dro["Col17Data"], 0);
                double col21 = Utils.ConvertToDouble(dro["Col24Data"], 0);
                ...
                double col32 = Utils.ConvertToDouble(dro["Col32Data"], 0);


                double col1 = col2 + col3 + col4 + col5 + col6 + col7;
                double col12 = col13 + col18 + col19 + col20 + col21;
                double col26 = col27 + col28;
                double col29 = col30 + col31 + col32;
                ...
                RowItem RowItem32 = new RowItem(32, Utils.AddCommasDouble(dro["Col31Data"].ToString()), dro["CoQuanID"].ToString(), dro["CapID"].ToString(), null, "text-align: center;" + Css, ref DataArr);
                RowItem RowItem33 = new RowItem(33, Utils.AddCommasDouble(dro["Col32Data"].ToString()), dro["CoQuanID"].ToString(), dro["CapID"].ToString(), null, "text-align: center;" + Css, ref DataArr);
        ...
        public List<TKDonThuInfo> XLD03_GetDSChiTietDonThu(string ContentRootPath, DateTime startDate, DateTime endDate, int start, int pagesize, int? Index, int? xemTaiLieuMat, int? canBoID, int coQuanID, int? capID)
        {
        ...
        public string XLD03_Excel(string rootPath, string pathFile, List<TableData> data, DateTime tuNgay, DateTime denNgay)
        {
            //var camera = new Camera;
            // path to your excel file
            string path = rootPath + @"\Templates\BaoCao\XLD03.xlsx";
        ...
        public void CalculateBaoCaoCapTinh(string ContentRootPath, int RoleID, int CoQuanDangNhapID, int CanBoDangNhapID, int TinhDangNhapID, ref List<BaoCao2bInfo> resultList, DateTime startDate, DateTime endDate, List<CapInfo> capList, bool flag, int tinhID, ref DataTable dt)
        {
            ...
            if (capList.Count > 0)
            {
                ...
                if (cqList.Count > 0)
                {
                    ...
                    try
                    {
                        using (SqlDataReader dr = SQLHelper.ExecuteReader(SQLHelper.appConnectionStrings, CommandType.StoredProcedure, "v2_BaoCaoXLD03_New", parm))
                        {
                            dt.Load(dr);
        ...
        public List<TKDonThuInfo> GetDSChiTietDonThu(DateTime startDate, DateTime endDate, List<CoQuanInfo> ListCoQuan, int start, int pagesize, int? Index, int? xemTaiLieuMat, int? canBoID, int? capID)
        {
            ...
            try
            {
                using (SqlDataReader dr = SQLHelper.ExecuteReader(SQLHelper.appConnectionStrings, CommandType.StoredProcedure, "v2_BaoCaoXLD03_GetDSChiTietDonThu_New", parm))
                {
                    query.Load(dr);
                }
            }
