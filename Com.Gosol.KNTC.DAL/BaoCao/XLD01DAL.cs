...
{
    public class XLD01DAL
    {
        public List<TableData> XLD01(List<int> listCapChonBaoCao, string ContentRootPath, int RoleID, int CapID, int CoQuanDangNhapID, int CanBoDangNhapID, int TinhDangNhapID, int HuyenDangNhapID, DateTime startDate, DateTime endDate)
        {
            List<TableData> data = new List<TableData>();
            DataTable dt = BaoCao2b_DataTable();
            //lblStartDate.Text = Format.FormatDate(startDate);
            //lblStartDate_excel.Text = Format.FormatDate(startDate);
            //lblEndDate.Text = Format.FormatDate(endDate);
            //lblEndDate_excel.Text = Format.FormatDate(endDate);
            //lblCurrentYear.Text = startDate.Year.ToString();
            //lblCurrentYear_excel.Text = startDate.Year.ToString();

            //hdfCanBoID.Value = CanBoDangNhapID.ToString();
            ...
            foreach (DataRow dro in dt.Rows)
            {
                ...
                double col19 = Utils.ConvertToDouble(dro["Col19Data"], 0);
                double col20 = Utils.ConvertToDouble(dro["Col20Data"], 0);
                double col21 = Utils.ConvertToDouble(dro["Col21Data"], 0);
                ...
                double col9 = col11 + col12 + col13;
                double col18 = col19 + col20 + col21;
                double col22 = col23 + col24 + col25;
                ...
                RowItem RowItem2 = new RowItem(2, Utils.AddCommasDouble(dro["Col1Data"].ToString()), dro["CoQuanID"].ToString(), dro["CapID"].ToString(), null, "text-align: center;" + Css, ref DataArr);
                ...
                RowItem RowItem19 = new RowItem(19, Utils.AddCommas(col18), dro["CoQuanID"].ToString(), dro["CapID"].ToString(), null, "text-align: center;" + Css, ref DataArr);
                ...
                RowItem RowItem23 = new RowItem(23, Utils.AddCommas(col22), dro["CoQuanID"].ToString(), dro["CapID"].ToString(), null, "text-align: center;" + Css, ref DataArr);
                ...
                RowItem RowItem27 = new RowItem(27, "", dro["CoQuanID"].ToString(), dro["CapID"].ToString(), null, "text-align: center;" + Css, ref DataArr);
        ...
        public List<TKDonThuInfo> XLD01_GetDSChiTietDonThu(string ContentRootPath, DateTime startDate, DateTime endDate, int start, int pagesize, int? Index, int? xemTaiLieuMat, int? canBoID, int coQuanID, int? capID)
        {
        ...
        public string XLD01_Excel(string rootPath, string pathFile, List<TableData> data, DateTime tuNgay, DateTime denNgay)
        {
            //var camera = new Camera;
            // path to your excel file
            string path = rootPath + @"\Templates\BaoCao\XLD01.xlsx";
        ...
        public void CalculateBaoCaoCapTinh(string ContentRootPath, int RoleID, int CoQuanDangNhapID, int CanBoDangNhapID, int TinhDangNhapID, ref List<BaoCao2bInfo> resultList, DateTime startDate, DateTime endDate, List<CapInfo> capList, bool flag, int tinhID, ref DataTable dt)
        {
            ...
            if (capList.Count > 0)
            {
                ...
                if (cqList.Count > 0)
                {
                    ...
                    try
                    {
                        using (SqlDataReader dr = SQLHelper.ExecuteReader(SQLHelper.appConnectionStrings, CommandType.StoredProcedure, "v2_BaoCaoXLD01_New", parm))
                        {
                            dt.Load(dr);
                        }
                    }
        ...
        private static void TinhTong(ref BaoCao2bInfo totalInfo, BaoCao2bInfo addInfo)
        {
            Type type = totalInfo.GetType();
            ...
            foreach (PropertyInfo property in properties)
            {
                ...
                {
                    int newValue = Utils.ConvertToInt32(property.GetValue(totalInfo, null), 0) + Utils.ConvertToInt32(property.GetValue(addInfo, null), 0);
                    property.SetValue(totalInfo, newValue, null);
                    //XuatExel();
                }
            }
        }
        ...
        #region 2b
        private const string GET_DONTHU = "BaoCao2b_GetDonThu";
        private const string GET_DONTHU_BY_TINH = "BaoCao2b_GetDonThuByTinh";
        private const string GET_DSCHITIETDONTHU = "BaoCao2b_GetDSChiTietDonThu";
        private const string COUNT_DSCHITIETDONTHU = "BaoCao2b_CountDSChiTietDonThu";

        private const string INSERT = @"BaoCao2b_Insert";
        private const string UPDATE = @"BaoCao2b_Update";
        private const string GET_ALL_XA_BY_COQUANHUYEN = @"BaoCao2b_GetAllCapXaByHuyen";

        private const string PARM_BAOCAO2BID = @"BaoCao2bID";
        ...
        private const string PARM_COT29 = @"Cot29";
        private const string PARM_COT30 = @"Cot30";
        private const string PARM_COT31 = @"Cot31";
        ...
        private BaoCao2bInfo GetData2b(SqlDataReader rdr)
        {
            BaoCao2bInfo info = new BaoCao2bInfo();
            ...
            info.BaoCao2bID = Utils.ConvertToInt32(rdr["BaoCao2bID"], 0);
            ...
            info.Col16Data = Utils.ConvertToInt32(rdr["Cot16"], 0);
            info.Col17Data = Utils.ConvertToInt32(rdr["Cot17"], 0);
            info.Col18Data = Utils.ConvertToInt32(rdr["Cot18"], 0);
            ...
            info.Col29Data = Utils.ConvertToInt32(rdr["Cot29"], 0);
            info.Col30Data = Utils.ConvertToInt32(rdr["Cot30"], 0);
            info.Col31Data = Utils.ConvertToInt32(rdr["Cot31"], 0);
        ...
        private SqlParameter[] GetInsertParms()
        {
            SqlParameter[] parms = new SqlParameter[]{
                ...
                new SqlParameter(PARM_COT16,SqlDbType.Int),
                new SqlParameter(PARM_COT17,SqlDbType.Int),
                new SqlParameter(PARM_COT18,SqlDbType.Int),
                ...
                new SqlParameter(PARM_COT29,SqlDbType.Int),
                new SqlParameter(PARM_COT30,SqlDbType.Int),
                new SqlParameter(PARM_COT31,SqlDbType.Int),
        ...
        private void SetInsertParms(SqlParameter[] parms, BaoCao2bInfo cInfo)
        {
            ...
            parms[9].Value = cInfo.Col8Data;
            parms[10].Value = cInfo.Col9Data;
            parms[11].Value = cInfo.Col10Data;
            parms[12].Value = cInfo.Col11Data;
            parms[13].Value = cInfo.Col12Data;
            parms[14].Value = cInfo.Col13Data;
            parms[15].Value = cInfo.Col15Data;
            parms[16].Value = cInfo.Col16Data;
            parms[17].Value = cInfo.Col17Data;
            parms[18].Value = cInfo.Col19Data;
            ...
            parms[29].Value = cInfo.Col29Data;
            parms[30].Value = cInfo.Col30Data;
            parms[31].Value = cInfo.Col31Data;
            parms[32].Value = cInfo.GhiChu;
            parms[33].Value = cInfo.CanBoID;
        }

        private SqlParameter[] GetUpdateParms()
        {
            SqlParameter[] parms = new SqlParameter[]{
                new SqlParameter(PARM_BAOCAO2BID,SqlDbType.Int),
        ...
        private void SetUpdateParms(SqlParameter[] parms, BaoCao2bInfo cInfo)
        {

            parms[0].Value = cInfo.BaoCao2bID;
            ...
            parms[7].Value = cInfo.Col8Data;
            parms[8].Value = cInfo.Col9Data;
            parms[9].Value = cInfo.Col10Data;
            parms[10].Value = cInfo.Col11Data;
            parms[11].Value = cInfo.Col12Data;
            parms[12].Value = cInfo.Col13Data;
            parms[13].Value = cInfo.Col15Data;
            parms[14].Value = cInfo.Col16Data;
            parms[15].Value = cInfo.Col17Data;
            parms[16].Value = cInfo.Col18Data;
            parms[17].Value = cInfo.Col19Data;
            parms[18].Value = cInfo.Col20Data;
            parms[19].Value = cInfo.Col21Data;
            parms[20].Value = cInfo.Col22Data;
            parms[21].Value = cInfo.Col23Data;
            parms[22].Value = cInfo.Col24Data;
            parms[23].Value = cInfo.Col25Data;
            parms[24].Value = cInfo.Col26Data;
            parms[25].Value = cInfo.Col27Data;
            parms[26].Value = cInfo.Col28Data;
            parms[27].Value = cInfo.Col29Data;
            parms[28].Value = cInfo.Col30Data;
            parms[29].Value = cInfo.Col31Data;
            parms[30].Value = cInfo.GhiChu;
        }
        public int Insert(BaoCao2bInfo cInfo)
        {
        ...
        public int Update(BaoCao2bInfo cInfo)
        {
        ...
        public IList<BaoCao2bInfo> GetAllXaByCoQuanHuyen(int CoQuanID, int thang, int nam)
        {
            IList<BaoCao2bInfo> infoList = new List<BaoCao2bInfo>();
            SqlParameter[] parm = new SqlParameter[] {
            ...
            try
            {
                using (SqlDataReader dr = SQLHelper.ExecuteReader(SQLHelper.appConnectionStrings, CommandType.StoredProcedure, GET_ALL_XA_BY_COQUANHUYEN, parm))
                {
                    while (dr.Read())
                    {
                        BaoCao2bInfo info = GetData2b(dr);
                        infoList.Add(info);
                    }
                }
            }
        ...
        public IList<BaoCao2bDonThuInfo> GetDonThu(DateTime startDate, DateTime endDate, int cqID)
        {
            ...
            try
            {
                ...
                //SqlDataReader dr = SQLHelper.ExecuteReader(SQLHelper.appConnectionStrings, CommandType.StoredProcedure, GET_DONTHU, parm);
                //DataTable result = new DataTable();
                //result.Load(dr);
                //var t = result.AsEnumerable();
                //t.Where(x => x.Field<int>("CoQuanID") == 1);
        ...
        public IList<BaoCao2bDonThuInfo> GetDonThu_Edit(DateTime startDate, DateTime endDate, int cqID)
        {
            var startDateInt = Utils.ConvertToInt32(
                                  startDate.Year.ToString()
                                  + (startDate.Month < 10 ? ("0" + startDate.Month.ToString()) : startDate.Month.ToString())
                                  + (startDate.Day < 10 ? ("0" + startDate.Day.ToString()) : startDate.Day.ToString())
                                  , 0);
            var endDateInt = Utils.ConvertToInt32(
            ...
            IList<BaoCao2bDonThuInfo> infoList = new List<BaoCao2bDonThuInfo>();
            SqlParameter[] parm = new SqlParameter[] {
                new SqlParameter(PARM_STARTDATE, SqlDbType.Int),
                new SqlParameter(PARM_ENDDATE, SqlDbType.Int),
                new SqlParameter(PARM_COQUANID, SqlDbType.Int)
            };
            parm[0].Value = startDateInt;
            parm[1].Value = endDateInt;
            parm[2].Value = cqID;
            try
            {
                using (SqlDataReader dr = SQLHelper.ExecuteReader(SQLHelper.appConnectionStrings, CommandType.StoredProcedure, "BaoCao2b_GetDonThu_Edit", parm))
                {
                    while (dr.Read())
                    {
                        BaoCao2bDonThuInfo info = GetData(dr);
        ...
        public IList<BaoCao2bDonThuInfo> GetDonThu_GetBy_ListCoQuan(DateTime startDate, DateTime endDate, List<int> listCoQuan)
        {
            var startDateInt = Utils.ConvertToInt32(
                                  startDate.Year.ToString()
                                  + (startDate.Month < 10 ? ("0" + startDate.Month.ToString()) : startDate.Month.ToString())
            ...
            var endDateInt = Utils.ConvertToInt32(
                ...
                , 0);
            IList<BaoCao2bDonThuInfo> infoList = new List<BaoCao2bDonThuInfo>();
            var plistCoQuanID = new SqlParameter(PARM_COQUANID, SqlDbType.Structured);
            plistCoQuanID.TypeName = "dbo.IntList";
            var table = new DataTable();
            table.Columns.Add("n", typeof(string));
            listCoQuan.ForEach(x => table.Rows.Add(x));
            SqlParameter[] parm = new SqlParameter[] {
                new SqlParameter(PARM_STARTDATE, SqlDbType.Int),
                new SqlParameter(PARM_ENDDATE, SqlDbType.Int),
                //new SqlParameter(PARM_COQUANID, SqlDbType.Int)
            ...
            try
            {
                using (SqlDataReader dr = SQLHelper.ExecuteReader(SQLHelper.appConnectionStrings, CommandType.StoredProcedure, "BaoCao2b_GetDonThu_GetBy_ListCoQuan", parm))
                {
                    while (dr.Read())
                    {
                        BaoCao2bDonThuInfo info = GetData(dr);
        ...
        public DataTable GetDSChiTietDonThu1(DateTime startDate, DateTime endDate, List<CoQuanInfo> ListCoQuan, int start, int pagesize, int? Index, int? xemTaiLieuMat, int? canBoID, int? capID)
        {

            DataTable dt = DanhSachChiTietTable();
            var pList = new SqlParameter("@ListCoQuanID", SqlDbType.Structured);
            pList.TypeName = "dbo.IntList";
            var tbCoQuanID = new DataTable();
            tbCoQuanID.Columns.Add("CoQuanID", typeof(string));
            ...
            //var k = ListCoQuan.Select(x => x.CoQuanID).ToList();
            try
            {
                using (SqlDataReader dr = SQLHelper.ExecuteReader(SQLHelper.appConnectionStrings, CommandType.StoredProcedure, "BaoCao2b_GetDSChiTietDonThu_New", parm))
                {
                    while (dr.Read())
                    {
                        DataRow drow = dt.NewRow();

                        drow["CoQuanID"] = dr["CoQuanID"];
                        drow["XuLyDonID"] = dr["CoQuanID"];
                        drow["DonThuID"] = dr["CoQuanID"];
                        drow["HuongGiaiQuyetID"] = dr["CoQuanID"];
                        drow["NgayNhapDon"] = dr["CoQuanID"];
                        drow["KetQuaID"] = dr["KetQuaID"];
                        drow["SoDonThu"] = dr["SoDonThu"];
                        drow["DiaChiCT"] = dr["DiaChiCT"];
                        ...
                        drow["TenLoaiKhieuTo"] = dr["TenLoaiKhieuTo"];
                        //  --   dtInfo.KetQuaID = Utils.ConvertToInt32(dr["KetQuaID"], 0);
                        drow["StateID"] = dr["StateID"];
                        drow["TenLoaiKhieuTo"] = dr["TenLoaiKhieuTo"];
                        drow["TenHuongGiaiQuyet"] = dr["TenHuongGiaiQuyet"];
                        if (Utils.ConvertToInt32(dr["HuongGiaiQuyetID"], 0) == 0)
                        {
                            drow["KetQuaID_Str"] = "Chưa giải quyết";
                        }
                        else if (Utils.ConvertToInt32(dr["HuongGiaiQuyetID"], 0) == (int)HuongGiaiQuyetEnum.DeXuatThuLy && Utils.ConvertToInt32(dr["StateID"], 0) != 10)
                        {
                            drow["KetQuaID_Str"] = "Đang giải quyết";
                        }
                        else
                        {
                            drow["KetQuaID_Str"] = "Đã giải quyết";
                        }
                        List<FileHoSoInfo> fileYKienXL = new List<FileHoSoInfo>();
                        List<FileHoSoInfo> fileBanHanhQD = new List<FileHoSoInfo>();
                        if (Utils.ConvertToInt32(drow["XuLyDonID"], 0) > 0)
                        {
                            if (xemTaiLieuMat == 1)
                            {

                                fileYKienXL = new FileHoSoDAL().GetFileYKienXuLyByXuLyDonID(Utils.ConvertToInt32(drow["XuLyDonID"], 0)).ToList();
                            }
                            else
                            {
                                fileYKienXL = new FileHoSoDAL().GetFileYKienXuLyByXuLyDonID(Utils.ConvertToInt32(drow["XuLyDonID"], 0)).Where(x => x.IsBaoMat != true || x.NguoiUp == canBoID).ToList();
                            }
                            ...
                            for (int i = 0; i < fileYKienXL.Count; i++)
                            {
                                ...
                                {
                                    string sec_false = step + fileYKienXL[i].FileURL + fileYKienXL[i].TenFile;
                                    drow["TenHuongGiaiQuyet"] = Utils.ConvertToString(drow["TenHuongGiaiQuyet"], string.Empty) + sec_false;
                                }
                                else
                                {
                                    string sec_true = step + fileYKienXL[i].FileURL + fileYKienXL[i].TenFile;
                                    drow["TenHuongGiaiQuyet"] = Utils.ConvertToString(drow["TenHuongGiaiQuyet"], string.Empty) + sec_true;
                                }
                            }
                        }

                        if (Utils.ConvertToInt32(drow["KetQuaID"], 0) > 0)
                        {
                            if (xemTaiLieuMat == 1)
                            {

                                fileBanHanhQD = new FileHoSoDAL().GetFileBanHanhQDByXuLyDonID(Utils.ConvertToInt32(dr["XuLyDonID"], 0)).ToList();
                            }
                            else
                            {
                                fileBanHanhQD = new FileHoSoDAL().GetFileBanHanhQDByXuLyDonID(Utils.ConvertToInt32(dr["XuLyDonID"], 0)).Where(x => x.IsBaoMat != true || x.NguoiUp == canBoID).ToList();
                            }
                            ...
                            for (int j = 0; j < fileBanHanhQD.Count; j++)
                            {
                                ...
                                {
                                    string sec_false = steps + fileBanHanhQD[j].FileURL + fileBanHanhQD[j].TenFile;
                                    drow["KetQuaID_Str"] = Utils.ConvertToString(dr["XuLyDonID"], string.Empty) + sec_false;
                                }
                                else
                                {
                                    string sec_true = steps + fileBanHanhQD[j].FileURL + ">" + fileBanHanhQD[j].TenFile;
                                    drow["KetQuaID_Str"] = Utils.ConvertToString(dr["XuLyDonID"], string.Empty) + sec_true;
                                }
                            }
                        }
                        dt.Rows.Add(drow);
            ...
            //    }
            //}
            //List<TKDonThuInfo> infoListNew = new List<TKDonThuInfo>();         
            //foreach (var i in infoList)
            //{
            //    var a = infoListNew.Where(x => x.DonThuID == i.DonThuID).ToList().Count;
            //    if ( a <= 0)
            //    {
            //        infoListNew.Add(i);
            //    }
            //}
            //infoList.GroupBy(x => x.DonThuID);
            return dt;
        }
        public List<TKDonThuInfo> GetDSChiTietDonThu(DateTime startDate, DateTime endDate, List<CoQuanInfo> ListCoQuan, int start, int pagesize, int? Index, int? xemTaiLieuMat, int? canBoID, int? capID)
        {
            ...
            try
            {
                using (SqlDataReader dr = SQLHelper.ExecuteReader(SQLHelper.appConnectionStrings, CommandType.StoredProcedure, "BaoCaoXLD01_GetDSChiTietDonThu", parm))
                {
                    query.Load(dr);
                }
            }
        ...
        public IList<BaoCao2bDonThuInfo> GetDonThuDongBo(DateTime startDate, DateTime endDate, int cqID, int tinhID)
        {
            IList<BaoCao2bDonThuInfo> infoList = new List<BaoCao2bDonThuInfo>();
            SqlParameter[] parm = new SqlParameter[] {
                new SqlParameter(PARM_STARTDATE, SqlDbType.DateTime),
