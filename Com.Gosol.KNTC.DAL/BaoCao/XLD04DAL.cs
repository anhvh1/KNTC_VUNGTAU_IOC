...
{
    public class XLD04DAL
    {
        public List<TableData> XLD04(List<int> listCapChonBaoCao, string ContentRootPath, int RoleID, int CapID, int CoQuanDangNhapID, int CanBoDangNhapID, int TinhDangNhapID, int HuyenDangNhapID, DateTime startDate, DateTime endDate)
        {
            ...
            foreach (DataRow dro in dt.Rows)
            {
                ...
                double col14 = Utils.ConvertToDouble(dro["Col15Data"], 0);
                double col15 = Utils.ConvertToDouble(dro["Col16Data"], 0);
                double col16 = Utils.ConvertToDouble(dro["Col17Data"], 0);
            
                double col22 = Utils.ConvertToDouble(dro["Col27Data"], 0);
                double col23 = Utils.ConvertToDouble(dro["Col28Data"], 0);

                double col1 = col2 + col3 + col4 + col5 + col6 + col7;
                double col12 = col13 + col14 + col15 + col16;
        ...
        public List<TKDonThuInfo> XLD04_GetDSChiTietDonThu(string ContentRootPath, DateTime startDate, DateTime endDate, int start, int pagesize, int? Index, int? xemTaiLieuMat, int? canBoID, int coQuanID, int? capID)
        {
        ...
        public string XLD04_Excel(string rootPath, string pathFile, List<TableData> data, DateTime tuNgay, DateTime denNgay)
        {
            //var camera = new Camera;
            // path to your excel file
            string path = rootPath + @"\Templates\BaoCao\XLD04.xlsx";
        ...
        public void CalculateBaoCaoCapTinh(string ContentRootPath, int RoleID, int CoQuanDangNhapID, int CanBoDangNhapID, int TinhDangNhapID, ref List<BaoCao2bInfo> resultList, DateTime startDate, DateTime endDate, List<CapInfo> capList, bool flag, int tinhID, ref DataTable dt)
        {
            ...
            if (capList.Count > 0)
            {
                ...
                if (cqList.Count > 0)
                {
                    ...
                    try
                    {
                        using (SqlDataReader dr = SQLHelper.ExecuteReader(SQLHelper.appConnectionStrings, CommandType.StoredProcedure, "v2_BaoCaoXLD04_New", parm))
                        {
                            dt.Load(dr);
        ...
        public List<TKDonThuInfo> GetDSChiTietDonThu(DateTime startDate, DateTime endDate, List<CoQuanInfo> ListCoQuan, int start, int pagesize, int? Index, int? xemTaiLieuMat, int? canBoID, int? capID)
        {
            ...
            try
            {
                using (SqlDataReader dr = SQLHelper.ExecuteReader(SQLHelper.appConnectionStrings, CommandType.StoredProcedure, "v2_BaoCaoXLD04_GetDSChiTietDonThu_New", parm))
                {
                    query.Load(dr);
                }
            }
