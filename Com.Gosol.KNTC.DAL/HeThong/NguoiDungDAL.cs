...
namespace Com.Gosol.KNTC.DAL.HeThong
{
    public class NguoiDungDAL
    {
            ...
            try
            {
                ...
                using (SqlDataReader dr = SQLHelper.ExecuteReader(SQLHelper.appConnectionStrings, CommandType.StoredProcedure, "v2_HT_NguoiDung_GetByLogin", parameters))
                {
                    if (dr.Read())
                    {
                        ...
                        user.CapUBND = Utils.ConvertToInt32(dr["CapUBND1"], 0);
        ...
        public NguoiDungModel GetInfoByRefreshToken(int nguoiDungID)
        {
            NguoiDungModel user = null;
            SqlParameter[] parameters = new SqlParameter[]
            {
                new SqlParameter(@"NguoiDungID",SqlDbType.NVarChar),
            };
            parameters[0].Value = nguoiDungID;
            try
            {

                using (SqlDataReader dr = SQLHelper.ExecuteReader(SQLHelper.appConnectionStrings, CommandType.StoredProcedure, "v2_HT_NguoiDung_GetByRefreshToken", parameters))
                {
                    if (dr.Read())
                    {
                        user = new NguoiDungModel();
