...
namespace Com.Gosol.KNTC.DAL.HeThong
{
    ...
    {
        private const string INSERT = @"v2_HT_SystemConfig_Insert";
        private const string UPDATE = @"v2_HT_SystemConfig_Update";
        private const string DELETE = @"v2_HT_SystemConfig_Delete";
        private const string GETBYID = @"v2_HT_SystemConfig_GetByID";
        private const string GETBYNAME = @"v2_HT_SystemConfig_GetByKey";
        private const string GETLISTPAGING = @"v2_HT_SystemConfig_GetPagingBySearch";
        ...
        //public SystemConfigDAL(DbKNTCContext dbKNTCContext)
        //{
        ...
        public BaseResultModel Insert(SystemConfigModel SystemConfigModel)
        {
            ...
            try
            {
                ...
                {
                    Result.Status = 0;
                    Result.Message = "Độ dài của ConfigKey không được quá 200 ký tự";
                }

                else if (SystemConfigModel.ConfigValue == null || SystemConfigModel.ConfigValue.Trim().Length < 1)
                {
                    Result.Status = 0;
                    Result.Message = "ConfigValue không được trống";
                }
                else if (SystemConfigModel.ConfigValue.Trim().Length > 200)
                {
                    Result.Status = 0;
                    Result.Message = "Độ dài của ConfigValue không được quá 200 ký tự";
                }
                else
                {
                    ...
                    {
                        Result.Status = 0;
                        Result.Message = "ConfigKey đã tồn tại";
                    }
                    else
                    {
        ...
        public BaseResultModel Update(SystemConfigModel SystemConfigModel)
        {
            ...
            try
            {
                ...
                {
                    Result.Status = 0;
                    Result.Message = "ConfigValue không được quá 200 ký tự";
                }
                else
                {
