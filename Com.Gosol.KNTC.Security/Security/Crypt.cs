...
namespace Gosol.Security
{
    public static class Crypt
    {
        private static string KeySecret_AES = "4073FB5F80884E6C8042CB0A8BD00KNTC0";//độ dài Key phải là 32 bit
        private static string vector_IV_AES = "8CA8B1831DF44960"; // độ dài vector IV_AES phải là 16 bit
        ...
        public static string EncryptStrings(string plainText, string KeySecret)
        {
            if (string.IsNullOrEmpty(plainText))
            {
                return plainText;
            ...
            using (AesCryptoServiceProvider aesAlg = new AesCryptoServiceProvider())
            {
                aesAlg.Key = Encoding.UTF8.GetBytes(KeySecret);
                aesAlg.IV = Encoding.UTF8.GetBytes(vector_IV_AES);
        ...
        public static string DecryptString(string cipherText, string KeySecret)
        {
            if (string.IsNullOrEmpty(cipherText))
            {
                return cipherText;
        ...
        public static string EncryptStrings_Aes(string key, string plainText)
        {
            if (key == null || (key != null && key.Length < 32))
            {
                return "";
            }
            if (string.IsNullOrEmpty(plainText))
        ...
        public static string DecryptString_Aes(string key, string cipherText)
        {
            if (key == null || (key != null && key.Length < 32))
            {
                return "";
