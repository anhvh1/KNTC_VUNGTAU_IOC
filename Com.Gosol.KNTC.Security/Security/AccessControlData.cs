namespace Com.Gosol.KNTC.Security
{
    ...
    internal sealed class AKNTCessControlData
    {
        ...
        //private static string SELECT_ACE_FROM_USER_ID = "Select ObjectTypeID, ObjectID, AKNTCessRight From [User] Inner Join UserGroup On [User].UserID = UserGroup.UserID Inner Join [Group] On UserGroup.GroupID = [Group].GroupID Inner Join ObjectACL On [Group].GroupID = EntityID Where ([Group].Status <> @GroupStatus) Group By ObjectID, ObjectTypeID, AKNTCessRight, [User].UserID Having [User].UserID = @UserID";
        ...
        //private static string SELECT_ACETYPE_FROM_USER_ID = "Select ObjectTypeID, AKNTCessRight From [User] Inner Join UserGroup On [User].UserID = UserGroup.UserID Inner Join [Group] On UserGroup.GroupID = [Group].GroupID Inner Join ObjectTypeACL On [Group].GroupID = EntityID Where ([Group].Status <> @GroupStatus) Group By ObjectTypeID, AKNTCessRight, [User].UserID Having [User].UserID = @UserID";
        ...
        private static Hashtable HashKeyValues(IDbConnection dbConnection, IDbCommand dbCommand, ObjectInfoDelegate objectInfoDelegate, ACLType aclType)
        {
            ...
            {
                throw new AKNTCessControlExceptions("Error at HashKeyValues method in DataAKNTCessManager class. Detail:" + exception.Message);
            }
            finally
            {
                dbConnection.Close();
        ...
        public static void RequestAKNTCessRight(int userId, out Hashtable acl, out Hashtable aclType)
        {
            ...
            //acl = HashKeyValues(dbConnection, dbCommand, new ObjectInfoDelegate(AKNTCessControlData.HashACLKeyValues), ACLType.ObjectInstance);
            acl = new Hashtable();
            dbCommand = DatabaseProxy.CreateDBCommand(dbConnection, SELECT_ACETYPE_FROM_USER_ID);
            DatabaseProxy.AddParameter(dbCommand, PARM_USER_ID, userId);
            aclType = HashKeyValues(dbConnection, dbCommand, new ObjectInfoDelegate(AKNTCessControlData.HashACLKeyValues), ACLType.ObjectClass);
        }

        public static void RequestUserInfo(int userID, out Hashtable userInfo, out Hashtable aKNTCessGroup)
        {
            IDbConnection dbConnection = DatabaseProxy.CreateDBConnection();
            ...
            DatabaseProxy.AddParameter(dbCommand, PARM_LOCKED, Convert.ToInt32(UserStatus.Locked));
            DatabaseProxy.AddParameter(dbCommand, PARM_USER_ID, userID);
            aKNTCessGroup = HashKeyValues(dbConnection, dbCommand, null, ACLType.ObjectList);
        }
        ...
        public static bool VerifyUser(string userName, string password, out int userID)
        {
            ...
            {
                throw new AKNTCessControlExceptions("DatabaseException");
            }
            finally
            {
                dbConnection.Close();
