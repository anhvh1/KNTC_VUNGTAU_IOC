...
namespace Com.Gosol.KNTC.API.Authorization
{
    ...
    public class CustomAuthAttribute : AuthorizeAttribute, IAsyncAuthorizationFilter
    {
        ...
        {
            if (CheckAKNTCessLevelByClaims(filterContext))
            {
        ...
        public bool CheckAKNTCessLevelByClaims(AuthorizationFilterContext filterContext)
        {
            try
            {
        ...
        {
            var isAuth = await Task.Run(() => CheckAKNTCessLevelByClaims(context));
            if (!isAuth)
            {
